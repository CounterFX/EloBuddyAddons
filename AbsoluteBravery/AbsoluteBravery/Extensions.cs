...
namespace AbsoluteBravery
{
    static class Extensions
    {
        ...
        /// <summary>
        /// Returns the gold cost from an item.
        /// </summary>
        public static float GetItemGoldCost(this ItemId item)
        {
            return new Item(item, 0).GoldRequired();
        }
        ...
        #region Randomize
        public static ItemId RandomItemIdFromList(this List<ItemId> list)
        {
            int index = ran.Next(list.Count - 1);

            return list[index];
        }

        public static SpellSlot RandomSpellSlotFromList(this List<SpellSlot> list)
        {
            int index = ran.Next(list.Count - 1);
